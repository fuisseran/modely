...
function oneToMany(modelName, args) {
  ...
  args.source.column !== null) {
      ...
      Modely.relationships[modelName][origin] = {
        ...
        join: function (knexObj) {
          if (typeof knekObj === 'undefined') {
            return ' LEFT INNER JOIN ' + origin + ' ON ' + args.source.column.fullname + ' = ' +
            args.target.column.fullname + ' '
          }
          knexObj.innerJoin(origin, args.source.column.fullname, args.target.column.fullname)
        }
      }
    } else {
    ...
    Modely.log.error('Unableto parse the relationship for model "%s"', modelName)
