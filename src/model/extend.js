...
// This should call the save on the parent class then and pass it's save as a transaction to the parent save.
module.exports = function (className, properties) {
  ...
  return new Promise(function(resolve,reject){
    ...
    if(typeof properties.columns[ParentName + '_' + ParentModel._primary_key] === 'undefined'){
      ...
      var primary_column = ParentModel._columns[ParentModel[ParentModel._primary_key]] 
      properties.columns[ParentName + '_' + primary_column.anme] = {
        name : className + '_' + primary_column.name,
        full_name : className + '.' + className + '_' + primary_column.name,
        table : className,
        type : primary_column.type,
        size : primary_column.size
      } 
    }
